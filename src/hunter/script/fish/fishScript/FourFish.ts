...
export default class FourFish extends AbstractFish {
    @inject(FightServer)
    ...
    setFishLoader() {
        ...
        for(let i = 0; i < combination.length; i++) {
            ...
            this.owner[`$owner`][`m_fourFish${i+1}Loader`].url = `ui://roomScene/fish_${DataMon.getData(combination[i]).uiId}_swining`;
            (this.owner[`$owner`][`m_fourFish${i+1}Loader`].content as fairygui.GMovieClip).setPlaySettings(0, -1, -1);
    ...
    frozenFish() {
        for(let i = 0; i < 4; i++) {
            (this.owner[`$owner`][`m_fourFish${i+1}Loader`] as fairygui.GLoader).playing = false;
        }
    }
    ...
    skewXFish(skewX: number) {
        for(let i = 0; i < 4; i++) {
            (this.owner[`$owner`][`m_fourFish${i+1}Loader`] as fairygui.GLoader).skewX = skewX;
        }
    }
    ...
    recover() {
        ...
        for(let i = 0; i < combination.length; i++) {
            (this.owner[`$owner`][`m_fourFish${i+1}Loader`] as fairygui.GLoader).playing = true;
            this.owner[`$owner`][`m_fourFish${i+1}Loader`].url = `ui://roomScene/fish_${DataMon.getData(combination[i]).uiId}_dead`;

            (this.owner[`$owner`][`m_fourFish${i+1}Loader`].content as fairygui.GMovieClip).setPlaySettings(0, -1, 3, -1, Laya.Handler.create(this, () => {
                j += 1;
